...
Public Class PingPool : Implements IDisposable

    Public Event PingEvent As PingEventHandler

    Public Property MaxWorkerCount As Integer = 16

    Private ReadOnly _available_pings As New Stack(Of PingWrapper)
    Private ReadOnly _lock As New Threading.AutoResetEvent(False)
    Private ReadOnly _active_pings As New List(Of PingWrapper)
    ...
    Public Sub EnqueuePing(address As IPAddress, ttl As Integer)
        SyncLock Me._update_lock
            Me._available_pings.Push(New PingWrapper(address, ttl))
        End SyncLock
    End Sub

    Public Sub EnqueuePings(pings As IEnumerable(Of PingWrapper))
        SyncLock Me._update_lock
            For Each p As PingWrapper In pings
        ...
        SyncLock Me._update_lock
            If Me._active_pings.Count < Me.MaxWorkerCount Then
                If Me._available_pings.Count > 0 Then
                    Dim new_ping As PingWrapper = Me._available_pings.Pop
    ...
    Private Sub _ping_call_back(sender As Object, e As PingEventArgs)
        Dim wrapper As PingWrapper = sender
        SyncLock Me._update_lock
    ...
    Private Sub _abort_pool()
        SyncLock Me._update_lock
            For Each p As PingWrapper In Me._active_pings
                p.Abort()
                p.Dispose()
            Next
